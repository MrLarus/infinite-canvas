...
```text
请从 https://github.com/MrLarus/infinite-canvas.git 安装 Infinite Canvas Codex 插件。
...
```bash
mkdir -p ~/plugins
git clone https://github.com/MrLarus/infinite-canvas.git ~/plugins/infinite-canvas
```
...
注意：当前 fork 仍沿用官方发布的 `@basketikun/canvas-agent` npm 包；仓库地址使用 MrLarus fork 是为了保留本项目的后端、登录、Otuapi 兼容和 DMIT 部署定制。
