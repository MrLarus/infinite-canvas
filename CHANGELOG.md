...
- [新增] 后端保留版进入 4.0 功能阶段，线上显示对齐官方 v0.4.0，同时保留 Go 后端、登录、管理后台、API 代理、WebDAV 自动同步和 DMIT Docker 部署链路。
- [新增] 选择性移植官方 4.0 相关能力：生成停止、网站 Agent、Agent 工具增强、工具确认摘要、文本模型选择和诊断日志。
- [新增] 后台渠道支持 Gemini 原生协议，普通用户请求继续通过后端代理，API Key 不下发到浏览器。
- [优化] 保留默认生图/生视频比例、自动记住比例、图片二抽快捷工具等本地定制能力。
- [调整] 不跟随官方 4.0 删除后端的纯前端个人版架构，后续官方更新继续采用逐提交审计和手工移植策略。
